//
//  PraxThumbnailView.swift
//
//  PraxPress - Prax=0104-1
//
//  Created by Elmer Cat on 1/4/26.
//

import AppKit
import PDFKit

final class PraxThumbnailView: NSCollectionView, NSCollectionViewDataSource, NSCollectionViewDelegate, NSCollectionViewDelegateFlowLayout {
    
    weak var pdfView: PDFView? {
        didSet {
            print("Uwandua pdfView didSet")
            reloadData()
        }
    }
...
//    private let collection = NSCollectionView()
    ...
    private let internalUTI = NSPasteboard.PasteboardType("com.praxpress.thumbnail-pages")
    ...
    private func commonInit() {
        ...
        self.register(ThumbCell.self, forItemWithIdentifier: itemIdentifier)
        self.dataSource = self
        self.delegate = self
        self.isSelectable = true
        self.setDraggingSourceOperationMask([.move], forLocal: true)
        self.setDraggingSourceOperationMask([.copy], forLocal: false)
        self.registerForDraggedTypes([internalUTI, .fileURL, .pdf])
 ...
 */
    ...
    override func layout() {
        super.layout()
        print("Uwandua override func layout()")
        //  scroll.frame = bounds
    }
 ...
 */
    ...
    func collectionView(_ collectionView: NSCollectionView, acceptDrop draggingInfo: NSDraggingInfo, indexPath: IndexPath, dropOperation: NSCollectionView.DropOperation) -> Bool {
        ...
        if let data = pb.data(forType: internalUTI), let array = try?
            NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? [Int] {
    ...
    func selectItemForCurrentPDFViewPage() {
        ...
        self.selectItems(at: [IndexPath(item: idx, section: 0)], scrollPosition: .nearestHorizontalEdge)
    }
}
