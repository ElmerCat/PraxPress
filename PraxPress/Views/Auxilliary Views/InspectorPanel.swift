...
extension View {
    /** Present a ``InspectorPanel`` in SwiftUI fashion
     - Parameter prax: The PraxModel instance to pass to the panel content
    ...
    func inspectorPanel<Content: View>(
        ...
        self.modifier(InspectorPanelModifier(isPresented: isPresented, prax: prax, contentRect: contentRect, view: content))
    }
}
...
fileprivate struct InspectorPanelModifier<PanelContent: View>: ViewModifier {
    ...
    /// The PraxModel instance to pass to the panel
    let prax: PraxModel
    ...
    func body(content: Content) -> some View {
        content
            .onAppear {
                ...
                panel = InspectorPanel(view: view, contentRect: contentRect, isPresented: $isPresented, prax: prax)
                panel?.center()
                if isPresented {
                    present()
                }
...
class InspectorPanel<Content: View>: NSPanel {
    ...
    /// The PraxModel instance to make available to the panel content
    let prax: PraxModel
    ...
    init(view: () -> Content,
        ...
        super.init(contentRect: contentRect,
                   styleMask: [.docModalWindow, .hudWindow, .titled, .resizable, .closable],
                    backing: backing,
                    defer: flag)
