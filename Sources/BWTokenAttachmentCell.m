...
+ (void)initialize
{
	//NSColor *blueTopColor = [NSColor redColor];  // Change color
...
- (NSImage *)arrowInHighlightedState:(BOOL)isHighlighted
{
	...
	NSImage *arrowImage = [NSImage imageNamed:@"NSSmartBadgeTemplate"];
...
- (void)drawTokenWithFrame:(NSRect)aRect inView:(NSView *)aView;
{
	...
	NSBezierPath *drawingPath = [NSBezierPath bezierPathWithRoundedRect:drawingRect xRadius:0.2*drawingRect.size.height yRadius:0.2*drawingRect.size.height];
	NSBezierPath *insetPath = [NSBezierPath bezierPathWithRoundedRect:insetRect xRadius:0.2*insetRect.size.height yRadius:0.2*insetRect.size.height];
	NSBezierPath *insetPath2 = [NSBezierPath bezierPathWithRoundedRect:insetRect2 xRadius:0.2*insetRect2.size.height yRadius:0.2*insetRect2.size.height];
    ...
    if ([[self tokenBackgroundColor] respondsToSelector:@selector(getRed:green:blue:alpha:)]) {
...
/*- (NSRect)pullDownRectForBounds:(NSRect)bounds
{
	...
	NSRect selfBounds = [self drawingRectForBounds:NSMakeRect(0, 0, cellSize.width, cellSize.height)];
//    pullDownRect.origin.x += pullDownRect.size.width;
//    pullDownRect.origin.x -= selfBounds.size.width;
//    pullDownRect.origin.y -= selfBounds.origin.y;
//    pullDownRect.size = selfBounds.size;
