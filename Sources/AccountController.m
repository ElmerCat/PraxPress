...
- (IBAction)downloadNew:(id)sender {
    ...
    [self.accountsPanel orderOut:sender];
    Account *account = self.accountArrayController.selectedObjects[0];
    [self.document.requestController reloadAccount:account option:PRAXReloadOptionAccount replace:NO];
}
...
- (IBAction)synchronizeDown:(id)sender {
    ...
    [self.accountsPanel orderOut:sender];
    Account *account = self.accountArrayController.selectedObjects[0];
    [self.document.requestController reloadAccount:account option:PRAXReloadOptionAccount replace:YES];
}
